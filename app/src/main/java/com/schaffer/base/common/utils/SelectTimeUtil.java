...
public class SelectTimeUtil {
    ...
    private static SelectTimeUtil mSelectTimeUtils;

    public static SelectTimeUtil getInstance() {
        if (mSelectTimeUtils == null) {
            mSelectTimeUtils = new SelectTimeUtil();
        }
    ...
    public void showChooseTimeDialog(final Context context, final TextView mTvTime) {
    ...
    private void showChooseTime(Context context, TextView mTvTime, int options1, int options2, int options3) {
        ...
        Date choose = null;
        try {
            choose = DateTimeUtil.formatStringToDate(chooseDate + chooseTime, "yyyy年MM月dd日 HH时mm分");

            long l = DateTimeUtil.formatDateToTimeStamp(choose);
            long l1 = DateTimeUtil.formatDateToTimeStamp(new Date());
            if (l < l1) {
                LogToastUtil.showShort(context, "选择的时间不能早于当前时间");
                return;
            }
            String timePicked = DateTimeUtil.formatDateToString(choose,"yyyy-MM-dd HH:mm");//转换成 yyyy-MM-dd HH:mm
            if (mTvTime == null) {
            } else {
