...
public class PagingItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public PagingItemDecoration(Context context, PageDecorationLastJudge pageDecorationLastJudge) {
        ...
        mDivider.setColorFilter(Color.parseColor("#ff6e62"), PorterDuff.Mode.CLEAR);
        a.recycle();
    }

    @Override
    public void onDraw(Canvas c, RecyclerView parent, State state) {
//        c.drawColor(Color.parseColor("#ff6e62"));
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, State state) {
        ...
        if (mPageDecorationLastJudge.isPageLast(itemPosition)) {
            outRect.set(0, 0, 0, 0);
        } else if (mPageDecorationLastJudge.isLastRow(itemPosition)) {
            // 如果是最后一行，则不需要绘制底部
            outRect.set(0, 0, mDivider.getIntrinsicWidth(), 0);
        } else {
            if (mPageDecorationLastJudge.isLastColumn(itemPosition)) {
                // 如果是最后一列，则不需要绘制右边
