 ...
 * helper.setUpRecycleView(mRvRedPockets);
 * helper.setOnPageChangeListener(new PagingScrollHelper.onPageChangeListener() {
 *
 * @Override public void onPageChange(int index) {
 * showLog("--->" + index);
 * }
 * });
 * <p>
 * HorizontalPageLayoutManager layoutManager = new HorizontalPageLayoutManager(2, 4);//行数,列数
 * PagingItemDecoration decoration = new PagingItemDecoration(this, layoutManager);
 * mRvRedPockets.setLayoutManager(layoutManager);
 * mRvRedPockets.addItemDecoration(decoration);
 */
